...
export const getWeather = () => async (next) => {
    ...
    const response = await fetch('https://api.openweathermap.org/data/2.5/weather?q=Busan&appid=2a1916c10e2b8385ff68acf916a2a813');
...
function weatherThunk (state=initalState, action) {
    switch(action.type) {
        case "getWeather":
            // 주소를 통해서 값 가져옴
            // 비동기로 값을 가져오기 때문에(시간차) 값이 들어가지 않는다
